...
`save(data, days?)` сужает диапазон до изменённых дней:

- `start`/`end` сужается до `min(days)..max(days)` — API перезаписывает только этот диапазон
- **Слоты включают ВСЕ дни в диапазоне `min..max`**, не только dirty — иначе бэкенд удалит промежуточные дни (баг: выходные [18, 26] сбрасывали дни 19–25)
